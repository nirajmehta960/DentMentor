...
const Onboarding = () => {
  ...
  }, [user, userType, mentorProfile, isProfileLoading, refreshProfile]);
  ...
  const step4Data = useOnboardingStepPersistence(
    4,
    "mentor",
    mentorProfile || {}
  );
  const step5Data = useOnboardingStepPersistence(
    5,
    "mentor",
    mentorProfile || {}
  );
  ...
  const handleNext = async (stepData: any) => {
    ...
    if (success) {
      if (nextStep > 5) {
        ...
        if (!isEditMode) {
          const completionSuccess = await updateMentorProfile(
