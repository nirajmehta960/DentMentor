-- Migration: Enable Idempotent Bookings
-- Description: Makes idempotency_key unique per mentee instead of globally, and updates create_booking_hold to handle retries safely.

-- 1. Update Constraints on booking_reservations
-- Drop the existing unique constraint if it exists (assuming it was a table-level constraint named booking_reservations_idempotency_key_key or similar)
ALTER TABLE public.booking_reservations
DROP CONSTRAINT IF EXISTS booking_reservations_idempotency_key_key;

-- Also try dropping index if it was just an index
DROP INDEX IF EXISTS public.booking_reservations_idempotency_key_key;

-- Add partial UNIQUE index per mentee
CREATE UNIQUE INDEX IF NOT EXISTS booking_reservations_mentee_idem_key_uq
ON public.booking_reservations (mentee_user_id, idempotency_key)
WHERE idempotency_key IS NOT NULL;

-- Add optimization index for cleanup
CREATE INDEX IF NOT EXISTS booking_reservations_status_expires_idx
ON public.booking_reservations (status, expires_at);

-- 2. Update create_booking_hold RPC
...
BEGIN
  -- A. Idempotency Check
  -- Check if we already have a valid reservation for this mentee + key
  IF p_idempotency_key IS NOT NULL THEN
    ...
    IF v_existing_reservation.id IS NOT NULL THEN
       -- If it's already held or pending_payment and NOT expired, return it
       ...
       ELSE
          -- The key exists but the reservation is expired or cancelled.
          -- We cannot create a new reservation with the same key due to unique constraint.
          -- We must tell the client to generate a new key.
...
EXCEPTION 
  WHEN unique_violation THEN
    -- If we hit a unique violation, it means a reservation with this key was created 
    -- concurrently by another transaction. We should return that one.
    SELECT * INTO v_existing_reservation
    FROM public.booking_reservations
    WHERE mentee_user_id = p_mentee_user_id
      AND idempotency_key = p_idempotency_key;
    ...
    ELSE
       RAISE; -- Should not happen if unique constraint violated
    END IF;
